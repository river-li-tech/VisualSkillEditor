...
void SkillEditor::onTreeScriptItemSelectionChanged()
{
    ...
    if (selectionList.length() > 0) {
        ...
        SkillScriptNode* _skillNode = (SkillScriptNode*)(item->data(0, Qt::UserRole + 1).value<void*>());
        if (_skillNode != nullptr) {
            loadWidgetParam(*_skillNode);
            selectTreeScriptNode(*_skillNode, false);
        }
